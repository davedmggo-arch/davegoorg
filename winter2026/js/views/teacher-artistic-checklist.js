/* ==========================================================================
   Seneca 3D Integration - js/views/teacher-artistic-checklist.js
   The TEACHER editor for the `artistic-checklist` collection (round 2.7).
   Mirrors teacher-shortcuts.js:

     - renderArtisticIndex  a search + category-chip index of ALL checklist
       rows (grouped by the 7 fixed categories, ordered), with per-row Edit /
       Delete, an Add button, and the standard saveBar.
     - renderArtisticEdit   a per-row schema form (id / category / text / order)
       with a Save-gate that rejects an empty text.
...
import { CATEGORIES } from './artistic-checklist.js';

const NAME = 'artistic-checklist';

/* The per-row schema. `category` is a select bound to the 7 fixed slugs. */
const SCHEMA = [
  { key: 'id', label: 'Row id', type: 'text', hint: 'Stable identifier, e.g. ac-01. Must be unique.' },
  { key: 'category', label: 'Category', type: 'select', options: CATEGORIES.map(function (c) { return { value: c.slug, label: c.label }; }) },
  { key: 'text', label: 'Checklist item', type: 'text', hint: 'One short self-check prompt.' },
  { key: 'order', label: 'Order', type: 'number', hint: 'Display position (kept dense, 1..N).' },
];

const BLANK_ROW = { id: '', category: 'silhouette', text: '', order: null };
...
function rowHtml(r) {
  return (
    '<li class="tgl-row">' +
      '<a class="tgl-row__term" href="#/admin/artistic-checklist/' + esc(r.id) + '">' + esc(r.text) + '</a>' +
      ...
      '<button class="tgl-row__del" type="button" data-acdel="' + esc(r.id) + '">Delete</button>' +
    '</li>'
  );
}
...
function resultsHtml(records, q, cat) {
  ...
  if (!filtered.length) return '<p class="tgl-empty muted">No checklist items match. Try another word or category.</p>';
  let html = '';
...
export function renderArtisticIndex() {
  ...
  const html = (
    '<section class="admin-edit tgl" aria-labelledby="acd-h">' +
      '<header class="admin-edit__head">' +
        '<h1 id="acd-h" class="anim-rise" style="--i:0">Artistic checklist</h1>' +
        '<p class="muted anim-rise" style="--i:1">' + records.length + ' items across ' + CATEGORIES.length + ' fixed categories. Search, filter, then open a row to edit. Add / Delete keeps the order dense (1..' + records.length + ').</p>' +
        '<div class="admin-edit__toolbar anim-rise" style="--i:1">' +
          '<button class="btn btn--primary" type="button" data-acadd>+ Add item</button>' +
          '<button class="btn btn--ghost" type="button" data-export="' + esc(NAME) + '">Export checklist</button>' +
        '</div>' +
      '</header>' +
      '<div class="tgl-tools anim-rise" style="--i:1">' +
        '<label class="sr-only" for="acd-search">Search checklist</label>' +
        '<input id="acd-search" class="search" type="search" autocomplete="off" placeholder="Search ' + records.length + ' items..." />' +
        '<span id="acd-count" class="tgl-count" role="status" aria-live="polite">' + countText(records, '', 'all') + '</span>' +
      '</div>' +
      '<div class="tgl-chips anim-rise" style="--i:1" role="group" aria-label="Filter by category">' + chips + '</div>' +
      '<div id="acd-results" class="tgl-results">' + resultsHtml(records, '', 'all') + '</div>' +
  ...
  if (view) {
    view.innerHTML = html;
    wireForm(view, { name: NAME, getRecord: function () { return {}; }, reRender: function () { renderArtisticIndex(); }, onChange: updateStatus });
...
function paint() {
  ...
  const results = document.getElementById('acd-results');
  if (results) results.innerHTML = resultsHtml(records, q, activeCat);
  const countEl = document.getElementById('acd-count');
...
function wireIndex() {
  ...
  window.addEventListener('click', function (e) {
    ...
    if (t.closest('[data-acadd]')) {
      e.preventDefault();
      const list = AdminData.get(NAME) || [];
      const blank = JSON.parse(JSON.stringify(BLANK_ROW));
      blank.id = nextId(list);
      blank.text = 'New checklist item';
      blank.order = (list.length ? Math.max.apply(null, list.map(function (r) { return r.order || 0; })) : 0) + 1;
      list.push(blank);
      redensify(list);
      Router.go('#/admin/artistic-checklist/' + blank.id);
      return;
    }

    if (t.closest('[data-acdel]')) {
      e.preventDefault();
      const id = t.closest('[data-acdel]').getAttribute('data-acdel');
      confirmDialog('Delete this checklist item ("' + id + '")?', { title: 'Delete item?', danger: true, okLabel: 'Delete' })
        .then(function (ok) {
          if (!ok) return;
          const list = AdminData.get(NAME) || [];
...
export function renderArtisticEdit(params) {
  const id = params && params.id;
  const view = (typeof document !== 'undefined') ? document.getElementById('view') : null;
  if (!view) return null;

  const rec = findRow(AdminData.get(NAME) || [], id);
  if (!rec) { view.innerHTML = notFound('Checklist item', 'id ' + id); return null; }

  const html = (
    '<section class="admin-edit" aria-labelledby="acd-edit-title">' +
      '<header class="admin-edit__head">' +
        '<h1 id="acd-edit-title" class="anim-rise" style="--i:0">Checklist item &middot; ' + esc(rec.text || rec.id) + '</h1>' +
      ...
      '<div class="admin-edit__body anim-rise" style="--i:1">' +
        ...
        '<div class="tgl-conflict" id="acd-conflict" role="status" aria-live="polite"></div>' +
      '</div>' +
      saveBar(NAME, '#/admin/artistic-checklist', 'Back to checklist') +
    '</section>'
  );
  ...
  wireOnce(view, 'savegate', { click: { fn: function (e) {
    ...
    const ind = document.getElementById('acd-conflict');
    if (bad.length) {
      e.preventDefault();
  ...
  wireForm(view, {
    ...
    reRender: function () { renderArtisticEdit(params); },
    onChange: updateStatus,
  });
  return null;
}

export default { renderArtisticIndex, renderArtisticEdit, SCHEMA, redensify, nextId, invalidFields };
